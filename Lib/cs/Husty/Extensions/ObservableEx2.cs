...
public static class ObservableEx2
{
    ...
    )
    {
        var observable = Observable
            .Repeat(0, scheduler ?? ThreadPoolScheduler.Instance)
    ...
    )
    {
        var counter = 0;
        var observable = Observable
            .Interval(interval, scheduler ?? ThreadPoolScheduler.Instance)
            .TakeUntil(_ => ct.IsCancellationRequested)
            .Select(_ => counter++)
            .Publish();
        observable.Connect();
    ...
    {
        var observable = Observable
            .Repeat(0, int.MaxValue, scheduler ?? ThreadPoolScheduler.Instance)
            .TakeUntil(_ => ct.IsCancellationRequested)
    ...
    {
        var observable = Observable
            .Range(0, int.MaxValue, scheduler ?? ThreadPoolScheduler.Instance)
            .TakeUntil(_ => ct.IsCancellationRequested)
            .Do(i => action(i))
            .Publish();
        observable.Connect();
